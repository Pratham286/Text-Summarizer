...
export const relationWithUser = async (req, res) => {
  try {
    ...
    if (user.pendingFriendReq.includes(otherUserId)) {
      ...
      return res
        .status(200)
        .json({ message: "Friend request received from user", status });
    }
    if (user.pendingFriendReqSent.includes(otherUserId)) {
      status.requestSent = true;
      return res
        .status(200)
        .json({ message: "Friend request sent to user", status });
    }
...
export const getFriendList = async (req, res) => {
  try {
    const user = req.user;
    const userDetails = await User.findById(user.id).populate("friends", "username fName lName");
    console.log(userDetails);
    if(!userDetails) {
      return res.status(404).json({ message: "User not found" });
    }
    return res.status(200).json({ message: "Friend list fetched successfully", friends: userDetails.friends });
  } catch (error) {
